...
from .network_agent import RepeatVector3D
import numpy as np
import os
import random
...
class AdvancedCoLightAgent(Agent):
    ...
    def convert_state_to_input(self, s):
        ...
        for i in range(self.num_agents):
            ...
            for idx, feat_name in enumerate(used_feature):
                if feat_name == "cur_phase":
                    if self.dic_traffic_env_conf["BINARY_PHASE_EXPANSION"]:
                        feats[idx].append(self.dic_traffic_env_conf['PHASE'][s[i][feat_name][0]])
                    else:
                        feats[idx].append(s[i][feat_name])
                else:
                    feats[idx].append(s[i][feat_name])
        # [1, agent, dim]
        feats2 = np.concatenate([np.array([ff]) for ff in feats], axis=-1)
        print(feats2.shape)
        # [1, agent, nei, agent]
    ...
    def prepare_Xs_Y(self, memory):
        ...
        for i in range(slice_size):
            ...
            for j in range(self.num_agents):
                ...
                _state[j].append([state[used_feature[0]] + state[used_feature[1]] + state[used_feature[2]]])
                _next_state[j].append([next_state[used_feature[0]] + next_state[used_feature[1]] +
                                       next_state[used_feature[2]]])
            _adjs.append(_adj)
